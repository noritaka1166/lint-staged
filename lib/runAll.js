...
export const runAll = async (
  ...
  if (abortController.signal.aborted || ctx.errors.size > 0) {
    logger.log(`${figures.cancelled} ${dim(`Skipped running tasks…`)}`)
  } else {
    await git.runTasks(ctx, tasks, { abortController, concurrent })
  }

  if (updateIndexSkipped(ctx)) {
    logger.log(`${figures.cancelled} ${dim('Skipped staging changes from tasks…')}`)
  } else {
  ...
  if (shouldRestoreUnstagedChanges(ctx)) {
    if (restoreUnstagedChangesSkipped(ctx)) {
      logger.log(`${figures.cancelled} ${dim('Skipped restoring unstaged changes…')}`)
    } else {
  ...
  if (shouldRestoreUntrackedFiles(ctx)) {
    if (restoreUnstagedChangesSkipped(ctx)) {
      logger.log(`${figures.cancelled} ${dim('Skipped restoring untracked files…')}`)
    } else {
  ...
  if (cleanupEnabled(ctx)) {
    if (cleanupSkipped(ctx)) {
      logger.log(`${figures.cancelled} ${dim('Skipped cleaning up temporary files…')}`)
    } else {
